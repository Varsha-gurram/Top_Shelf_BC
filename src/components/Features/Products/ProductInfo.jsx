...
const ProductInfo = ({
  ...
  const selectedWeightObj = weightOptions.find(opt => opt.label === selectedWeight) || weightOptions[0];
          ...
          <Box>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              {weightOptions.map((opt) => (
                <Button key={opt.label} size="small"
                  onClick={() => setSelectedWeight(opt.label)}
                  sx={{
                    minWidth: 0, px: 2, fontWeight: 600, borderRadius: 2, fontSize: 14,
